...
using Riff.Droid.UI.MusicLibrary;
...
namespace Riff.Droid.UI
{
    ...
    {
        private Riff.Droid.Sync.OneDriveSyncServiceConnection syncServiceConnection = null;
        ...
        protected async override void OnStart()
        {
            ...
            if (!this.syncServiceConnection.IsConnected && !app.Preferences.IsSyncPaused)
            {
                ...
                if (loginExists)
                {
                    ...
                    Intent serviceIntent = new Intent(this, typeof(Riff.Droid.Sync.OneDriveSyncService));
        ...
        private void SyncEngine_StateChanged(object sender, Riff.Sync.SyncState e)
        {
            RunOnUiThread(() =>
            {
        ...
        private void UpdateSyncStateIcon(IMenu menu, Riff.Sync.SyncState state)
        {
            if (menu == null)
            {
                return;
            ...
            if (state == Riff.Sync.SyncState.Uptodate)
            {
                menuItem.SetIcon(Resource.Drawable.ic_sync_uptodate);
            }
            else if (state == Riff.Sync.SyncState.Syncing)
            {
            ...
            else if (state == Riff.Sync.SyncState.Stopped)
            {
                menuItem.SetIcon(Resource.Drawable.ic_sync_disabled);
            }
            else if (state == Riff.Sync.SyncState.NotSyncing)
            {
